...
const { getUserName } = require("./osDependent");
const { enableSshHostBasedAuthentication, keyScan, installAnsible, pullRolesFromGalaxy, runPlaybooks } = require("../../common/setupScripts");
const { getPlaybook, getPlaybookNames } = require("../../common/playbook");
const { maskSensitiveValues } = require("../../common/util.js");
const { setupIAMRole, setupKeyPair, setupVPC, getImage, cleanUp, cleanUpIAMRole, cleanUpKeyPair, createEC2Object, createIAMObject } = require("./internal.js");
...
function parseOrder(order) {
  ...
  const runJobOnBatchServer = order.runJobOnBatchServer;
  const mpi = order.mpi;
  const compiler = order.compiler;
  const playbook = order.playbook;
  debug({ os, numberOfNodes, shareStorage, rootVolume, batch, mpi, compiler });
  ...
  return [os, id, pw, numberOfNodes, headOnlyParam, rootVolume, shareStorage, publicKey, batch, runJobOnBatchServer, mpi, compiler, playbook, remaining];
}
...
async function awsCreate(argOrder) {
  ...
  //get ami id
  const image = await getImage(os, region, ec2);
  ...
  const headUserDataObject = {
    ...
    runcmd: [
      ...
      installAnsible(os),
      ...makeInventory,
      pullRolesFromGalaxy(shareStorage, batch, mpi, compiler),
      runPlaybooks(user, playbookDir, shareStorage, batch, mpi, compiler, userPlaybook)
    ],
    write_files: getPlaybookNames(shareStorage, batch, mpi, compiler).map((e)=>{
      let opt;
      if (e === "pbspro") {
        opt = runJobOnBatchServer;
      }
      return {
  ...
  if (userPlaybook) {
    headUserDataObject.write_files.push({
      path: `${playbookDir}/userPlaybook.yml`,
      mode: 755,
      content: `${userPlaybook}`
    });
  }
  ...
  await Promise.all([
    cleanUpKeyPair(ec2, clusterID),
    cleanUpIAMRole(iam, clusterID)
  ]);
