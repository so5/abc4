...
async function awsCreate(order) {
  ...
  const playbooks = ["nfs", "test"];
  const pullPlaybookFromGalaxy = "ansible-galaxy install -p/etc/ansible/roles geerlingguy.nfs andrewrothstein.nfs-client";
  const runPlaybook = `sudo -i -u ${user} ANSIBLE_SSH_CONTROL_PATH=/tmp/  ansible-playbook ${playbooks.reduce((a, c)=>{
