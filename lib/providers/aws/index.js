...
async function awsCreate(argOrder) {
  ...
  const shostsEquiv = "/etc/ssh/shosts.equiv"; //default for ubuntu
  const shostsRoot = "/root/.shosts";

  commonOpt.makeShostsEquiv = [
    ...
    |tr '\\t' '\\n' | awk 'BEGIN{FS="."} {print $0} /internal/{print $1}' \
    |tee ${shostsEquiv} |tee ${shostsRoot}`
  ];
  ...
  if (Array.isArray(commonOpt.packages)) {
    commonOpt.packages.push("python", "awscli");
  } else {
    commonOpt.packages = ["python", "awscli"];
  }
...
async function getInstances(region, Filters, accessKeyId, secretAccessKey) {
  const ec2 = createEC2Object(region, accessKeyId, secretAccessKey);
...
async function getClusterInstances(clusterID, accessKeyId, secretAccessKey) {
  ...
  try {
    ...
    return getAllInstances(clusterID, accessKeyId, secretAccessKey);
  }
  ...
  return getInstances(region, Filters, accessKeyId, secretAccessKey);
}
