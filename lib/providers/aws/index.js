...
const { enableSshHostBasedAuthentication, keyScan, installAnsible } = require("../../common/setupScripts");
...
function parseOrder(order) {
  ...
  return [os, region, id, pw, numberOfNodes, headOnlyParam, rootVolume, shareStorage, publicKey, batch, mpi, remaining];
}
...
async function awsCreate(order) {
  ...
  const playbooks = ["nfs", "pbspro"];
  const pullPlaybookFromGalaxy = "ansible-galaxy install -p/etc/ansible/roles geerlingguy.nfs andrewrothstein.nfs-client so5.pbspro";
  const runPlaybook = `sudo -i -u ${user} ANSIBLE_SSH_CONTROL_PATH=/tmp/  ansible-playbook --timeout=3600 ${playbooks.reduce((a, c)=>{
    return `${a} ${playbookDir}/${c}.yml`;
  }, "")}`;
  ...
  const headUserDataObject = {
    ...
    runcmd: [
      ...
      installAnsible(os),
      makeInventory,
      pullPlaybookFromGalaxy,
      runPlaybook
    ],
    write_files: playbooks.map((e)=>{
      return {
