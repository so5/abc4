...
const { getSshHostBasedAutScript, keyScan } = require("../../common/setupScripts");
...
async function awsCreate(order) {
  ...
  const sshHostbasedAuth = getSshHostBasedAutScript(os);

  const playbooks = "test.yml";
  ...
  const headUserDataObject = {
    ...
    runcmd: [
      ...
      makeInventory,
      `cd ${playbookDir} && sudo -u ${user} ansible-playbook ${playbooks}`
    ],
    write_files: [
      {
        path: `${playbookDir}/test.yml`,
        owner: `${user}:${group}`,
