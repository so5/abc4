...
- hosts: all
  ...
  if (opt.hasOwnProperty("qmgrCmds") && Array.isArray(opt.qmgrCmds)) {
    vars += `    pbspro_qmgr_cmds: [ ${opt.qmgrCmds.map((e)=>{
      return `"${e}"`;
    }).join(",")} ]\n`;
  }
  if (opt.hasOwnProperty("runJobOnBatchServer")) {
 ...
 * @param {string} opt - Common option argument extracted from order object.
...
/**
 * Return playbook names.
 * @param {string} opt - Common option argument extracted from order object.
 * @returns {string[]} - Playbook names to be used.
 */
...
/**
 * Return array of playbooks name which should be run locally on each node.
 * @param {string} opt - Common option argument extracted from order object.
 * @returns {string[]} - Playbook names to be used.
 */
...
/**
 * Return role names which should be downloaded from ansible-galaxy.
 * @param {string} opt - Common option argument extracted from order object.
 * @returns {string[]} - Playbook names to be used.
 */
