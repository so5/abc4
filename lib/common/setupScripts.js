...
function getUserData(opt, isChild) {
  ...
  const userData = {
    runcmd: [
      ...
      `ansible-playbook ${opt.playbookDir}/setupHostbasedAuth.yml --extra-vars="{ssh_hostbased_auth_permit_root_login: yes, ssh_hostbased_auth_allowed_hosts: [\`sed 's/.*/"&"/' ${opt.ipListFile} | tr '\\n' ',' |sed 's/,$//'\`]}"`,
  ...
  const makeSureAllNodeIsUp = "start=`date +%s`;until ansible -m ping all || [ $(( `date +%s` - $start )) >600 ]; do sleep 5;done";
  ...
  if (!isChild) {
    userData.runcmd.push(
      ...
      makeSureAllNodeIsUp,
      `for i in \`cat ${opt.ipListFile}\`;do ssh-keyscan $i >> ~${opt.user}/.ssh/known_hosts;done`,
