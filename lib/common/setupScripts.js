...
function pullRolesFromGalaxy(shareStorage, batch, mpi, compiler) {
  const roles = getRoleNames(shareStorage, batch, mpi, compiler);
  return `ansible-galaxy install -p/etc/ansible/roles ${roles.join(" ")}`;
}

function runPlaybooks(user, playbookDir, shareStorage, batch, mpi, compiler, userPlaybook) {
